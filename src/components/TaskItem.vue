<template>
  <label class="flex items-center mx-4">
    <input
      type="checkbox"
      name="Urgent"
    ...
    <div class="flex items-center justify-between w-full">
      ...
      <section class="flex flex-col md:flex-row items-center justify-center text-center ml-4">
        ...
        <svg :class="{'hidden': task.urgency <= 0}" class="text-red-500 mx-3 h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default {
  ...
  methods: {
    ...
    getTaskTime(task) {
      let time = new Date(task.created_at)
